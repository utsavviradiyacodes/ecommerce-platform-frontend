...
import { hasMeaningfulSubcategoryChanges } from "../../features/subcategories/subcategoryFormUtils.js";
...
const LIVE_COMPARISON_FIELD_NAMES = ["name", "categoryId"];
...
function SubcategoryFormModal({
  ...
  imageError = "",
  submitError = "",
  categoryDependencyMessage = "",
  isCategorySelectionDisabled = false,
  isCategorySelectionLoading = false,
  isSubmitting = false,
  ...
  const modalTitle = isEditMode ? "Edit subcategory" : "Add subcategory";

  const modalDescription = isEditMode
    ? "Update the subcategory name, parent category, or replace its current image."
    : "Create a subcategory under one of your existing product categories.";

  const submitLabel = isEditMode ? "Save changes" : "Add subcategory";

  const displayedPreviewUrl = selectedPreviewUrl || previewUrl;

  const [currentName = "", currentCategoryId = ""] = useWatch({
    control,
    name: LIVE_COMPARISON_FIELD_NAMES,
  });

  const hasMeaningfulChanges =
    !isEditMode ||
    hasMeaningfulSubcategoryChanges(
      {
        name: currentName,
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    hint={categoryError || categoryDependencyMessage}
                    disabled={isSubmitting || isCategorySelectionDisabled}
                    isSearchable={categoryOptions.length > 0}
                    searchPlaceholder="Search categories..."
                    isLoading={isCategorySelectionLoading}
            ...
            <div className="order-1 md:order-2">
              ...
              <div className="mt-4 hidden rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 sm:block dark:border-gray-800 dark:bg-white/2">
                <p className="text-xs leading-5 text-gray-500 dark:text-gray-400">
                  ...
                  beside the subcategory name throughout the admin panel.
                </p>
              </div>
            </div>
          </div>
