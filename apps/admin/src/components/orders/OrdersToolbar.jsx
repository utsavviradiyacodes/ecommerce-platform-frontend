...
function OrdersToolbar({
  ...
  const safeMatchingCount = getSafeCount(matchingCount);
  const resultLabel =
    safeMatchingCount === 1
      ? "1 matching order"
      : `${safeMatchingCount.toLocaleString("en-IN")} matching orders`;

  return (
    <div className="mb-5 min-w-0 rounded-xl border border-gray-200 bg-white p-4 sm:p-5 dark:border-white/5 dark:bg-white/3">
      <div className="grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-[minmax(0,1fr)_minmax(0,180px)_minmax(0,180px)_minmax(0,180px)]">
        <div className="min-w-0">
          <div className="relative min-w-0">
            <label htmlFor="order-search" className="sr-only">
              Search Orders on the current page
            </label>
            <span
            ...
            <input
              ...
              placeholder="Search this page by Order, customer, or seller..."
